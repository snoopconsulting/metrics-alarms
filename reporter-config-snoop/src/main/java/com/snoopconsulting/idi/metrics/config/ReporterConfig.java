...
public class ReporterConfig {
	private static final Logger log = LoggerFactory
			.getLogger(ReporterConfig.class);
	...
	@Valid
	private List<AlarmReporterConfig> alarm;

	public List<AlarmReporterConfig> getAlarm() {
		return alarm;
	}

	public void setAlarm(List<AlarmReporterConfig> alarm) {
		this.alarm = alarm;
	}
	...
	public boolean enableAlarm() {
		boolean failures = false;
		if (alarm == null) {
			log.debug("Asked to enable alarm, but it was not configured");
			return false;
		}
		for (AlarmReporterConfig alarmConfig : alarm) {
	...
	public static ReporterConfig loadFromFileAndValidate(String fileName)
			throws IOException {
	...
	public static ReporterConfig loadFromFile(String fileName)
			throws IOException {
	...
	public static <T> boolean validate(T obj) {
		...
		final Set<ConstraintViolation<T>> violations = factory.getValidator()
				.validate(obj);
	...
	public void setAlarm(Alarm alarmTest) {
		for (AlarmReporterConfig a : this.getAlarm()) {
			a.setAlarm(alarmTest);
		}
	}
